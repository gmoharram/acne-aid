...
class SkinSegmentationNN(pl.LightningModule):
    ...
    def __init__(self, num_classes=3, hparams=None):
        ...
        self.loss = nn.CrossEntropyLoss(ignore_index=-1, reduction='mean')  # pixel loss function where target class void is ignored
        
        resnet = torchvision.models.resnet18(pretrained=True)  # download pretrained resnet18 model
        resnet_used_layers = nn.Sequential(*list(resnet.children())[:-3]) # remove last 3 layers from pretrained resnet18 model
        if hparams['num_resnet_trainable'] > 0:
            self.resnet_trainable = resnet_used_layers[-hparams['num_resnet_trainable']:] #make last n layers of remaining resnet network trainable
        ...
        self.decoder = nn.Sequential(  # define trainable "decoder" architecture to train on skin segmenation task
            ...
            nn.ReLU(inplace=True),
            nn.ConvTranspose2d(num_classes, num_classes, kernel_size=(36,20), stride=14)
        )
    ...
    def general_step(self, batch, batch_idx, mode):
        ...
        targets = targets - 1  # shift all category ids by -1 such that not-ignored classes start at 0, artifact of annotation tool
