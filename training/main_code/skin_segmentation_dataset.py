...
class SkinSegmentationData(data.Dataset):
    ...
    def __init__(self, image_paths_file, input_height = 680, input_width = 510):
    ...
    def get_item_from_index(self, index):
        ...
        #img = center_crop(img)
        resizer = transforms.Resize(size=self.input_size, interpolation=transforms.InterpolationMode.NEAREST)
        img = resizer(img)
        img = to_tensor(img)
