...
class CameraManager: ObservableObject {
    ...
    private func setupVideoInput() {
        do {
            let deviceTypes: [AVCaptureDevice.DeviceType] = [.builtInDualCamera, .builtInTelephotoCamera,  .builtInWideAngleCamera, .builtInUltraWideCamera]
    ...
    func captureImage() {
        sessionQueue.async { [weak self] in
            ...
            // Deprecated: photoSettings.isHighResolutionPhotoEnabled = true
...
class CameraDelegate: NSObject, AVCapturePhotoCaptureDelegate {
    ...
    func photoOutput(_ output: AVCapturePhotoOutput, didFinishProcessingPhoto photo: AVCapturePhoto, error: Error?) {
        ...
        if let imageData = photo.fileDataRepresentation(), let capturedImage = UIImage(data: imageData) {
            // here you can specify what happens to the captured image
