...
from database.querying import insert_record, get_record, get_records_by_field
from database.storing import upload_progress_image, download_progress_image
...
async def upload_image(
    ...
    if experiment_record.user_id != user_id:
        return {
            "message": "Error: User attempting to upload image to an experiment belonging to another user."
        }

    storage_bucket = "progress.skin"
    image_path = f"{user_id}/{experiment_id}/{image.filename}_{datetime.today().strftime('%Y-%m-%d')}"
